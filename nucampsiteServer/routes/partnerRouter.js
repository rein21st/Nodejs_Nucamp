...
.post(authenticate.verifyUser,(req, res) => {
    res.statusCode = 403;
    res.send(`POST operation not supported on /partner/${req.params.partnerId}`);
})
...
.put(authenticate.verifyUser,(req, res) => {
    res.statusCode = 403;
    res.send(`PUT operation not supported on /partners/${req.params.partnerId}/comments`);
})
.delete(authenticate.verifyUser,(req, res, next) => {
    Partner.findByIdAndDelete(req.params.partnerId)
    .then(partner => {
...
.get((req, res, next) => {
    Partner.findById(req.params.partnerId)
    .then(partner => {
        if (partner.comments.id(req.params.commentId)) {
            res.statusCode = 200;
            res.setHeader('Content-Type', 'application/json');
            ...
            err = new Error(`Partner ${req.params.partnerId.commentId} not found`);
            err.status = 404;
            return next(err);
        }
    })
