...
.put(authenticate.verifyUser,(req, res) => {
    res.statusCode = 403;
    res.send(`PUT operation not supported on /promotions/${req.params.promotionId}/comments`);
})
.delete(authenticate.verifyUser,(req, res, next) => {
    Promotion.findById(req.params.promotionId)
    .then(promotion => {
...
.get((req, res, next) => {
    Promotion.findById(req.params.promotionId)
    .then(promotion => {
        if (promotion.comments.id(req.params.commentId)) {
            res.statusCode = 200;
            res.setHeader('Content-Type', 'application/json');
            ...
            err = new Error(`Promotion ${req.params.promotionId.commentId} not found`);
            err.status = 404;
            return next(err);
        }
    })
